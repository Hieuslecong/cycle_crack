...
Gram(x) = x @ x.T / (C × H × W)
```
**CRITICAL paper detail**: The paper states `l = 3` (a single VGG layer).
Use **VGG-16** features at layer index 3 (`relu1_2` equivalent).
Freeze VGG weights (`requires_grad=False`).

**Do NOT** use VGG-19 or multiple layers (relu1_1 to relu4_1) — that is NOT in the paper.
...
```
# Attention mask from discriminator intermediate features
M_attn = sigmoid(||F(I_C) - F(G_E(I_C))||)
# where F(·) = intermediate features from D_N at layer 4
# Apply sigmoid element-wise after computing L1 norm across channel dim

# Region loss
L_region = ||∇G_E(I_C) - ∇I_C||_1                     # Sobel gradient term
         + ||( G_E(I_C) - I_C ) ⊙ (1 - M_attn)||_1    # pixel diff in non-crack region
```

Where `∇` is the **Sobel** gradient operator implemented with `F.conv2d` (differentiable, fixed kernels).

**Paper formulation** (Eq.12) — do NOT replace with L2 loss or other variants.
...
- [ ] `D_N.forward()` returns `(output, feat)` tuple
- [ ] VGG frozen: `param.requires_grad == False` for all VGG params
- [ ] `L_text` called for **both** `(I_C, I''_C)` and `(I_N, I''_N)` in train loop
- [ ] `L_region` called **once** (G_E branch only) in train loop
- [ ] `.detach()` present when computing `loss_D`
- [ ] Patch filter uses `> 100` threshold (not `> 0`)
- [ ] `lambda_text=2.0`, `lambda_region=1.0` in config
- [ ] `LR_G=1e-4`, `LR_D=4e-4` in config
- [ ] Scheduler is `CosineAnnealingLR` (not `LambdaLR`)
- [ ] `infer.py` runs end-to-end without morphological closing (unless flagged)
